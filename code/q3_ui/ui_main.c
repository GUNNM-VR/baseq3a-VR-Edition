...
#else
DLLEXPORT intptr_t vmMain(int command, int arg0, int arg1, int arg2, int arg3) {
#endif
	switch ( command ) {
...
static cvarTable_t		cvarTable[] = {
	...
	{ &laserBeamRGBA, "laserBeamRGBA", "FF0000C0", CVAR_ARCHIVE },
};
...
void UI_VideoCheck( int time ) 
{
	if ( abs( time - uis.lastVideoCheck ) > 1000 ) {
		...
		oldWidth = uis.glconfig.menuWidth;
		oldHeight = uis.glconfig.menuHeight;

		trap_GetGlconfig( &uis.glconfig );

		if ( uis.glconfig.menuWidth != oldWidth || uis.glconfig.menuHeight != oldHeight ) {
			uis.biasY = 0.0;
			uis.biasX = 0.0;
			// for 640x480 virtualized screen
			if ( uis.glconfig.menuWidth * 480 > uis.glconfig.menuHeight * 640 ) {
				// wide screen, scale by height
				uis.scale = uis.glconfig.menuHeight * (1.0/480.0);
				uis.biasX = 0.5 * ( uis.glconfig.menuWidth - ( uis.glconfig.menuHeight * (640.0 / 480.0)));
			} else {
				// no wide screen, scale by width
				uis.scale = uis.glconfig.menuWidth * (1.0/640.0);
				uis.biasY = 0.5 * ( uis.glconfig.menuHeight - ( uis.glconfig.menuWidth * (480.0 / 640)));
			}
