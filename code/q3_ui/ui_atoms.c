...
=================
*/
void UI_MouseEvent( int dx, int dy, int dz )
{
	int				i;
	menucommon_s*	m;
	...
	if ( !vr_controller_type.integer ) {
		...
		// show a curseur at beam intersection with the virtual menu
		uis.cursorx = dx;
		uis.cursory = dy;
	}
	uis.cursorz = dz;
	...
	uis.cursorx = UI_ClampCvar(uis.screenXmin, uis.screenXmax, uis.cursorx);
	uis.cursory = UI_ClampCvar(uis.screenYmin, uis.screenYmax, uis.cursory);
...
void UI_Refresh( int realtime )
{
	...
	if ( uis.activemenu )
	{
		...
		{
			// draw the background
			trap_R_DrawStretchPic( 0, 0, uis.glconfig.menuWidth, uis.glconfig.menuHeight, 0, 0, 1, 1, uis.menuBackNoLogoShader );
	...
	// draw cursor
	if (!lb_init) {
		hex_to_color( laserBeamRGBA.string, client_beam_RGBA );
		lb_init = qtrue;
	}

	color_rgba[0] = (float)(client_beam_RGBA[0] / 255.0f);
	color_rgba[1] = (float)(client_beam_RGBA[1] / 255.0f);
	color_rgba[2] = (float)(client_beam_RGBA[2] / 255.0f);
	color_rgba[3] = (float)(client_beam_RGBA[3] / 255.0f);

	//UI_DrawString(0, 0, va("color_rgba : %f %f %f %f", color_rgba[0], color_rgba[1], color_rgba[2], color_rgba[3]), UI_LEFT | UI_SMALLFONT, colorRed);

	UI_SetColor( color_rgba );
		
	if ( uis.cursorz ) // do not draw cursor when menu is not focused
