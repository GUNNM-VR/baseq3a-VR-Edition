...
static void PlayerSettings_DrawName( void *self ) {
	...
	{
		// when  virtual keyboard is shown, the name is always focused in this property page
		int showVirtualKeyboard = trap_Cvar_VariableValue( "showVirtualKeyboard" );
		
		if ( !focus && showVirtualKeyboard == 1 ) {
			focus = qtrue;
		}
	}
#endif	
...
#ifdef USE_VIRTUAL_KEYBOARD
	case ID_NAME:
		set_VR_focusItem((menucommon_s*)ptr);
		trap_Cvar_Set("showVirtualKeyboard", "1" );
		break;
#endif
	}
}
