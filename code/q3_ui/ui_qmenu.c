...
=================
*/
menucommon_s* keepFocusItem;

menucommon_s* get_VR_focusItem() {
	return keepFocusItem;
}

void set_VR_focusItem(menucommon_s *item) {
	keepFocusItem = item;
}
...
#ifdef USE_VIRTUAL_KEYBOARD
	// prevent cursor to focus another item when using virtual keyboard
	showVirtualKeyboard = (int)trap_Cvar_VariableValue( "showVirtualKeyboard" );
	if(showVirtualKeyboard && keepFocusItem)
		item = keepFocusItem;
	else
#endif
