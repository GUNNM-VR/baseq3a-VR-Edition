...
static unsigned int hex2int(const char *str)
{
	int i;
	int len = 2;
	unsigned long val = 0;
	...
	for (i = 0; i < len; i++)
		if (str[i] <= 57)
			val += (str[i] - '0')*(1 << (4 * (len - 1 - i)));
		else
			val += (str[i] - 'A' + 10)*(1 << (4 * (len - 1 - i)));
...
void hex_to_color(const char *str, byte *color_rgba) {
	...
	memcpy(tmp, &str[0], sizeof(tmp));
	color_rgba[0] = (byte)hex2int(tmp);

	memcpy(tmp, &str[2], sizeof(tmp));
	color_rgba[1] = (byte)hex2int(tmp);

	memcpy(tmp, &str[4], sizeof(tmp));
	color_rgba[2] = (byte)hex2int(tmp);

	memcpy(tmp, &str[6], sizeof(tmp));
	color_rgba[3] = (byte)hex2int(tmp);
}
#endif
