...
// Choose only one compilation type :
#define USE_VR_QVM // to build the non-native VR QVM
//#define USE_NATIVE_HACK // drBeef's native hack for compatibility with q3a servers, build the native QVM.

// Others options :
#define USE_NEOHUD // compile new HUD parser
#define USE_ITEM_TIMERS 1 // compile item timers, show how long until power-up respawns
#define USE_VIRTUAL_MENU
#define USE_LASER_SIGHT
#define USE_VIRTUAL_KEYBOARD
#define USE_GRAPPLING_HOOK
#define USE_WEAPON_WHEEL
#define USE_DEATHCAM

//GUNNM The following commands have not been tested
//#define USE_VR_ZOOM
//#define USE_TRAIL_ITEM
//#define USE_MEGAGIBS
//#define USE_CONFORT_VIGNETTE
//#define USE_VR_FOV
//#define USE_HAPTIC
//#define USE_HUD_FLAG


// let this one be automatic
#if defined USE_VR_QVM || defined USE_NATIVE_HACK
#define USE_VR
#endif

// this is use as an enum, keep it
...
void hex_to_color(const char *str, byte *color_rgba);
#endif
...
typedef struct playerState_s {
	...
	vec3_t		HMD_raw_origin;
	vec3_t  	right_muzzle_position; // needed for intersect with menu, TODO use translation according to weapon model
	vec3_t  	right_hand_angles; // angles come from PM_UpdateViewAngles()
	vec3_t		right_hand_position;
	vec3_t  	left_hand_position;
	vec3_t  	left_hand_angles; // angles come from PM_UpdateViewAngles()
	int			vrFlags;
#endif // USE_VR_QVM
} playerState_t;
...
typedef struct entityState_s {
	...
	trajectory_t 	rmpos;	// for calculating right hand weapon muzzle position
	trajectory_t  	rapos;	// for calculating right hand weapon angles
	vec3_t  		s_rightAngles;
	trajectory_t 	rpos;	// for calculating right hand position
	trajectory_t 	lpos;	// for calculating left hand position
