...
/* weapon_oculusgo */
	{
		"weapon_oculusgo", 
		"sound/misc/w_pkup.wav",
		{ "models/weapons2/oculusgo/oculusgo.md3",
		0, 0, 0},
		"icons/iconw_oculusgo", // icon
		"Oculus Go Controller", // pickup
		0, //default munition nb
		IT_WEAPON,
		WP_VR_CONTROLLER,
		"", // precache
		"" // sounds
...
qboolean BG_CanItemBeGrabbed( int gametype, const entityState_t *ent, const playerState_t *ps ) {
	...
	// avoid sound loop
	if((ent->eFlags & EF_NODRAW) && (ent->eFlags & EF_TIMER)) {
		return qfalse;
	}
#endif
...
void BG_PlayerStateToEntityState( playerState_t *ps, entityState_t *s, qboolean snap ) {
	...
	// HMD position origin
	//-------------------------
	s->hmdpos.trType = TR_INTERPOLATE;
	VectorCopy(ps->HMD_raw_origin, s->hmdpos.trBase);
	if (snap)
		SnapVector(s->hmdpos.trBase);

	//-------------------------
	// VR Right hand weapon muzzle position
	//-------------------------
	s->rmpos.trType = TR_INTERPOLATE;
	VectorCopy( ps->right_muzzle_position, s->rmpos.trBase );
	if ( snap )
		SnapVector( s->rmpos.trBase );

	//-------------------------
	// VR Right hand angles
	//-------------------------
	s->rapos.trType = TR_INTERPOLATE;
	VectorCopy(ps->right_hand_angles, s->rapos.trBase);
	if (snap)
		SnapVector(s->rapos.trBase);
	VectorCopy(ps->right_hand_angles, s->s_rightAngles);

	//-------------------------
	// VR Right hand position
	//-------------------------
	s->rpos.trType = TR_INTERPOLATE;
	VectorCopy(ps->right_hand_position, s->rpos.trBase);
	if (snap)
		SnapVector(s->rpos.trBase);

	//-------------------------
	// VR Left hand position
	//-------------------------
	s->lpos.trType = TR_INTERPOLATE;
	VectorCopy(ps->left_hand_position, s->lpos.trBase);
	if (snap)
		SnapVector(s->lpos.trBase);

	//-------------------------
	// VR Left hand angles
	//-------------------------
	s->lapos.trType = TR_INTERPOLATE;
	VectorCopy(ps->left_hand_angles, s->lapos.trBase);
	if (snap)
		SnapVector(s->lapos.trBase);
	VectorCopy(ps->left_hand_angles, s->s_leftAngles);

	s->vrFlags = ps->vrFlags;
#endif// USE_VR_QVM
...
void BG_PlayerStateToEntityStateExtraPolate( playerState_t *ps, entityState_t *s, int time, qboolean snap ) {
	...
	// HMD position
	//-------------------------
	s->hmdpos.trType = TR_LINEAR_STOP;
	VectorCopy(ps->HMD_raw_origin, s->hmdpos.trBase);
	if (snap)
		SnapVector(s->hmdpos.trBase);
	// set the time for linear prediction
	s->hmdpos.trTime = time;
	// set maximum extra polation time
	s->hmdpos.trDuration = 50; // 1000 / sv_fps (default = 20)

	//-------------------------
	// VR Right hand weapon muzzle position
	//-------------------------
	s->rmpos.trType = TR_LINEAR_STOP;
	VectorCopy( ps->right_muzzle_position, s->rmpos.trBase );
	if ( snap )
		SnapVector( s->rmpos.trBase );
	// set the time for linear prediction
	s->rmpos.trTime 	= time;
	// set maximum extra polation time
	s->rmpos.trDuration = 50; // 1000 / sv_fps (default = 20)
	...
	s->rpos.trTime = time;
	// set maximum extra polation time
	s->rpos.trDuration = 50; // 1000 / sv_fps (default = 20)

	//-------------------------
	// VR Left hand position
	//-------------------------
	s->lpos.trType = TR_LINEAR_STOP;
	VectorCopy(ps->left_hand_position, s->lpos.trBase);
	if (snap)
		SnapVector(s->lpos.trBase);
	// set the time for linear prediction
	s->lpos.trTime = time;
	// set maximum extra polation time
	s->lpos.trDuration = 50; // 1000 / sv_fps (default = 20)
