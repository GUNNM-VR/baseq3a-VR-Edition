...
#ifdef USE_NATIVE_HACK
	rotateAboutOrigin(vrSpace[0], vrSpace[1], ent->client->ps.viewangles[YAW] - vrinfo->hmdinfo.angles.actual[YAW], r);
#endif
...
#ifdef USE_NATIVE_HACK
	qboolean use_fake_6dof = qtrue;
	qboolean mySelf = (ent->client->ps.clientNum == vrinfo->clientNum);

	if (mySelf)
		vr_controller_type = vrinfo->vr_controller_type;
#endif

#ifdef USE_VR_QVM
	// Only non native VM send others players VR info
	vr_controller_type = ent->client->ps.vr_controller_type;
#endif
...
#ifdef USE_NATIVE_HACK
	else if ( vrinfo != NULL && mySelf )
	{
		float worldscale = trap_Cvar_VariableValue("vr_worldscale");
		convertFromVR(ent, vrinfo->controllers[SideRIGHT].hmd_ctrl_offset, ent->r.currentOrigin, muzzlePoint);

		if ( vr_controller_type >= 2 ) {
			muzzlePoint[2] -= ent->client->ps.viewheight; // remove q3 player height
			muzzlePoint[2] += vrinfo->hmdinfo.position.actual[1] * worldscale; // add real height from HMD
		}
	}
#endif

#ifdef USE_VR_QVM
	else if ( vr_controller_type )
	{
		float worldscale = trap_Cvar_VariableValue("vr_worldscale");
		//convertFromVR(ent, vrinfo->weaponoffset, ent->r.currentOrigin, muzzlePoint);

		vec3_t rightOffset;
		VectorSubtract(ent->client->ps.HMD_raw_origin, ent->client->ps.right_hand_position, rightOffset);

		convertFromVR(ent, rightOffset, ent->r.currentOrigin, muzzlePoint);
		muzzlePoint[2] -= ent->client->ps.viewheight;
		muzzlePoint[2] += ent->client->ps.HMD_raw_origin[1] * worldscale;
	}
#endif
...
void CalcMuzzlePointOrigin( gentity_t *ent, vec3_t origin, vec3_t forward, vec3_t right, vec3_t up, vec3_t muzzlePoint) {
#ifdef USE_VR
	qboolean vr_controller_type = qfalse;
	...
	if ( vr_controller_type > 0 )
	{
		//if (!(ent->client->ps.eFlags & EF_LEFT_HANDED)) // TODO
			VectorCopy(ent->client->ps.right_muzzle_position, origin);
		//else
		//	VectorCopy(ent->client->ps.left_muzzle_position, origin);
	}
	else
#endif // USE_VR_QVM
#endif // USE_VR
	...
	// snap to integer coordinates for more efficient network bandwidth usage
	if ( vr_controller_type != 0 )
		SnapVector( muzzlePoint );// was commented out in q3e.
#endif
}
...
/*#ifdef USE_NATIVE_HACK
	if ( vr_controller_type >= 2 )
	if ( !(ent->r.svFlags & SVF_BOT) && vrinfo != NULL &&
		mySelf &&
		//!vrinfo->use_fake_6dof && 
		!use_fake_6dof && 
		(vr_controller_type >= 2)
		)
	{
		//===================
		//		6 Dof
		//===================
		VectorCopy(vrinfo->controllers[SideRIGHT].angles.actual , angles);
		angles[YAW] += ent->client->ps.viewangles[YAW] - vrinfo->hmdinfo.angles.actual[YAW];
	}
	else
#endif*/
#ifdef USE_VR_QVM
	if (vr_controller_type > 0) {
		...
		//		3/6 Dof
		//===================
		VectorCopy(ent->client->ps.right_hand_angles, angles);
	}
	else
#endif
		//===================
...
static void Bullet_Fire( gentity_t *ent, float spread, int damage ) {
	...
	//VectorMA( muzzle_origin, ( 8192.0 * 16.0 ), forward, end );//ori q3e
	VectorMA( muzzle, ( 8192.0 * 16.0 ), forward, end );
	...
	for ( i = 0; i < 10; i++ ) {
		...
		//trap_Trace( &tr, muzzle_origin, NULL, NULL, end, passent, MASK_SHOT );//ori q3e
...
#ifdef USE_VR
#ifdef USE_VR_QVM
	if ( ent->client->ps.vr_controller_type > 0)
#endif
		// VR client
...
void FireWeapon( gentity_t *ent ) {
	qboolean isGrapple = qfalse;
#ifdef USE_VR
	vec3_t viewang;
	qboolean vr_controller_type;
#endif
	...
	// set aiming directions for VR controller
	// use right hand angles as view angles when possible
#ifdef USE_NATIVE_HACK
	if ( mySelf && (vr_controller_type == 1) ) {
		VectorCopy(vrinfo->controllers[SideRIGHT].angles.actual, viewang);
		// add the yaw spawn angle
		viewang[YAW] += ent->client->ps.viewangles[YAW] - vrinfo->hmdinfo.angles.actual[YAW];
	}
	else
#endif
#ifdef USE_VR_QVM
	if ( vr_controller_type > 0 )
		VectorCopy(ent->client->ps.right_hand_angles, viewang);
	else
#endif
