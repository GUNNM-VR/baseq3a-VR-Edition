...
void SetClientViewAngle( gentity_t *ent, vec3_t angle ) {
...
#ifdef USE_VR_QVM
	vr_controller_type = client->ps.vr_controller_type;
#endif
	...
	{
		// for correct view PITCH when respawn and previously looking up or down
		int	cmdAngle = ANGLE2SHORT(angle[YAW]);
		client->ps.delta_angles[YAW] = cmdAngle - client->pers.cmd.angles[YAW];
		ent->s.angles[YAW] = angle[YAW];
		client->ps.viewangles[YAW] = angle[YAW];
	}
	else
#endif
	{
...
/*
==================
SetClientVrControllerAngles
// and keep angle when TeleportPlayer
==================
*/
#ifdef USE_VR_QVM
void SetClientVrControllerAngles( gentity_t *ent, vec3_t angle ) {
	// set the delta angle
	int cmdAngle = ANGLE2SHORT(angle[YAW]);
	ent->client->ps.delta_spawn_angles[YAW] = cmdAngle - ent->client->pers.cmd.right_hand_angles[YAW];
	ent->s.s_rightAngles[YAW] = angle[YAW];
	ent->client->ps.right_hand_angles[YAW] = angle[YAW];
}
#endif
...
qboolean ClientUserinfoChanged( int clientNum ) {
	...
	// VR stuff
	vr_controller_type = atoi( Info_ValueForKey( userinfo, "vr_controller_type" ) );
	//laserBeamPlayer = atoi( Info_ValueForKey( userinfo, "laserBeam" ) );
	Q_strncpyz(c3, Info_ValueForKey( userinfo, "laserBeamRGBA" ), sizeof( c3 ));
	...
	if ( ent->r.svFlags & SVF_BOT ) {
		...
		/*s = va("n\\%s\\t\\%i\\model\\%s\\hmodel\\%s\\c1\\%s\\c2\\%s\\hc\\%i\\w\\%i\\l\\%i\\tt\\%d\\tl\\%d\\vt\\%i\\lb\\%i", // VR
//		s = va("n\\%s\\t\\%i\\model\\%s\\hmodel\\%s\\c1\\%s\\c2\\%s\\hc\\%i\\w\\%i\\l\\%i\\tt\\%d\\tl\\%d", // q3a
			client->pers.netname, client->sess.sessionTeam, model, headModel, c1, c2, 
			client->pers.maxHealth, client->sess.wins, client->sess.losses, teamTask, teamLeader,
			vr_controller_type, laserBeamPlayer );// VR*/

		s = va("n\\%s\\t\\%i\\model\\%s\\hmodel\\%s\\c1\\%s\\c2\\%s\\hc\\%i\\w\\%i\\l\\%i\\tt\\%d\\tl\\%d\\vt\\%i\\lb\\%i", // VR
//		s = va("n\\%s\\t\\%i\\model\\%s\\hmodel\\%s\\c1\\%s\\c2\\%s\\hc\\%i\\w\\%i\\l\\%i\\tt\\%d\\tl", // q3a
		client->pers.netname, client->sess.sessionTeam, model, headModel, c1, c2,
		client->pers.maxHealth, client->sess.wins, client->sess.losses, teamTask, teamLeader,
		vr_controller_type );// VR
	}
...
//	client->areabits = savedAreaBits;
...
#ifdef USE_VR_QVM
	client->ps.vr_controller_type = atoi(Info_ValueForKey(userinfo, "vr_controller_type"));
#endif

	SetClientViewAngle( ent, spawn_angles );

#ifdef USE_VR_QVM
	// set VR controller angle at spawn_angles
	// send spawn_angles to VR client
	SetClientVrControllerAngles( ent, spawn_angles );
#endif
	...
	if ( level.intermissiontime ) {
		...
		//for ( i = WP_NUM_WEAPONS - 1 ; i > 0 ; i-- ) {//q3A
		for ( i = WP_NUM_WEAPONS - 3 ; i > 0 ; i-- ) { // grapple and VR controller aren't selected by default
