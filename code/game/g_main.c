...
vmCvar_t 	g_itemTimer;
#endif
#ifdef USE_DEATHCAM
vmCvar_t 	g_allowDeathCam;
#endif
...
#ifdef USE_ITEM_TIMERS
	{ &g_itemTimer, "g_itemTimer", "1", CVAR_SERVERINFO },
#endif
#ifdef USE_DEATHCAM
	{ &g_allowDeathCam, "g_allowDeathCam", "1", CVAR_SERVERINFO },
#endif
...
#ifdef USE_GRAPPLING_HOOK
	,{ &g_PureAllowHook, "g_PureAllowHook", "0", CVAR_SERVERINFO | CVAR_LATCH, 0, qfalse }
#endif
};
...
static void G_InitGame( int levelTime, int randomSeed, int restart ) {
	...
	if ( value[0] ) {
#ifdef Q3_VM
		trap_GetValue = (void*)~atoi( value );// warning: conversion from `pointer to void' to `pointer to qboolean function
#else
...
void MoveClientToIntermission( gentity_t *ent ) {
	...
	if ( vr_controller_type ) {
		// VR client
		// adjust VR controller's angle
		float yawAngle;
		int cmdAngle;
		yawAngle = -180;
		yawAngle += level.intermission_angle[YAW];

		// set the delta angle
		cmdAngle = ANGLE2SHORT(yawAngle);
		ent->client->ps.delta_spawn_angles[YAW] = cmdAngle - ent->client->pers.cmd.right_hand_angles[YAW];
		ent->s.s_rightAngles[YAW] = yawAngle;
		ent->client->ps.right_hand_angles[YAW] = yawAngle;
	}
#endif
