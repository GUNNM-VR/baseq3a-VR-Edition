...
#ifdef USE_VR
		viewheight = cg.heightCrouch;
#else
		viewheight = CROUCH_VIEWHEIGHT;
#endif
	} else {
#ifdef USE_VR
		viewheight = cg.heightStand;
#else
...
//		trap_S_StartSound (NULL, es->number, CHAN_AUTO, cgs.media.noAmmoSound );
	...
	case EV_CHANGE_WEAPON:
		...
		if ( clientNum == cg.predictedPlayerState.clientNum ) {
			...
			vrinfo->realign = 3; // auto trigger weapon re-align in 4 frames
			//int position = vrinfo->weapon_stabilised ? 4 : (vrinfo->right_handed ? 1 : 2);
#endif
#ifdef USE_HAPTIC
	...
	case EV_PLAYER_TELEPORT_IN:
		...
		if (clientNum == cg.predictedPlayerState.clientNum) {
#ifdef USE_NATIVE_HACK
			vrinfo->realign = 3; // Initiate position reset for fake 6DoF
#endif
#ifdef USE_HAPTIC
		...
		if ( !cg.vr_controller_type ) 
			...
			if ( es->clientNum == cg.snap->ps.clientNum && !cg.renderingThirdPerson )
			{
				if ( cg.vr_controller_type >= 2 ) {
					...
					//==================
					vec3_t angles;
					CG_CalculateWeaponPosition_VR_6Dof( es->origin2, angles );
					VectorCopy(es->origin2, vec);
				}
				else if ( cg.vr_controller_type == 1 ) {
					//==================
					//		3 Dof
					//==================
					float fovOffset = -0.2f * (cgs.fov - 90.0f);
	...
	case EV_POWERUP_BATTLESUIT:
		//if ( es->number == cg.snap->ps.clientNum ) {//q3e
		if ( es->number == cg.predictedPlayerState.clientNum ) {//ioq3quest
