...
void CG_NewClientInfo( int clientNum ) {
	...
	v = Info_ValueForKey(configstring, "vt"); // vr_controller_type
	newInfo.vr_controller_type = atoi( v );

	//GUNNM TODO pass the vrFlags int
	//v = Info_ValueForKey(configstring, "lb"); // laserBeamPlayer
	//newInfo.laserBeamPlayer = atoi( v );


	//GUNNM TODO
	/*v = Info_ValueForKey(configstring, "c3"); // laserBeamRGBA
	CG_ColorFromRGBAString( v, newInfo.laserBeamRGBA );*/

	/*newInfo.c1RGBA[0] = 255 * newInfo.laserBeamRGBA[0];
	newInfo.c1RGBA[1] = 255 * newInfo.laserBeamRGBA[1];
	newInfo.c1RGBA[2] = 255 * newInfo.laserBeamRGBA[2];
	newInfo.c1RGBA[3] = 255* newInfo.laserBeamRGBA[3];*/
	...
	if ( !CG_ScanForExistingClientInfo( &newInfo ) ) {
		...
		myself = qfalse;// (clientNum == cg.snap->ps.clientNum); // no need to defer loading of the client / bug with native as cg.snap is NULL -------------------------------------------------------------------------------

		// if we are defering loads, just have it pick the first valid
		if ( forceDefer || (can_defer && !cg_buildScript.integer && !cg.loading) && !myself ) {
...
static void CG_PlayerAngles( centity_t *cent, vec3_t legs[3], vec3_t torso[3], vec3_t head[3] ) {
	...
	float			speed;
	int				dir, clientNum;
	qboolean 		isDead, fixedtorso, fixedlegs;
	int 			vr_controller_type;
	vec3_t			rightAngles;
	qboolean		firstPersonBody;
	qboolean		mySelf = (cent->currentState.number == cg.snap->ps.clientNum);

	isDead = fixedtorso = fixedlegs = qfalse;

#ifdef USE_VR
	//========================
	// Get weapon hand angles
	//========================
	VR_get_cent_controller_angles( cent, rightAngles, SideRIGHT );

	//qboolean firstPersonBody = mySelf && (!cg.renderingThirdPerson) && (cg_firstPersonBodyScale.value > 0.0f);
	firstPersonBody = mySelf && (!cg.renderingThirdPerson);

	if ( cg.vr_controller_type >= 2 && firstPersonBody )
	{
		//=====================================
		//				6 Dof
		//=====================================
		...
		vec3_t headAngles, legsAngles;
		float ctrlYaw;

		if ( mySelf ) {
			VR_get_client_HMD_angles( headAngles );
		}
		else if ( cg.snap ) {
			VectorCopy(cg.snap->ps.viewangles, headAngles);
		}

#ifdef USE_NATIVE_HACK
		ctrlYaw = rightAngles[YAW];
#else
		if ( cg.snap ) {
			ctrlYaw = cg.snap->ps.right_hand_angles[YAW];
		}
#endif

		VectorClear(legsAngles);

		legsAngles[YAW] = cg.refdefViewAngles[YAW] + headAngles[YAW] - ctrlYaw;

		// only show a fraction of the pitch angle in the legs
		...
		AnglesToAxis( rightAngles, torso );
	...
	if (clientNum >= 0 && clientNum < MAX_CLIENTS) {
		...
		if ( ci->infoValid ) {
			vr_controller_type = ci->vr_controller_type;
			fixedtorso = ci->fixedtorso;
			fixedlegs  = ci->fixedlegs;
		}
	}

	// if VR player is dead, only head movements are allowed
	isDead = ( cent->currentState.eFlags & EF_DEAD );

	if ( vr_controller_type == 1 && !isDead ) {
		torsoAngles[YAW]   = rightAngles[YAW];
		torsoAngles[PITCH] = rightAngles[PITCH];
		weaponAngles[ROLL] = rightAngles[ROLL];
	}
...
#if defined USE_VR
	if ( vr_controller_type == 1 ) {
		AnglesToAxis( weaponAngles, client_weapon.axis );
	}
	else
#endif
		AnglesToAxis( torsoAngles, client_weapon.axis );
}
...
void CG_TrailItem(centity_t *cent, qhandle_t hModel, vec3_t offset, float scale) {
	...
	if (cent->currentState.clientNum == cg.snap->ps.clientNum)
	{
		...
		//qboolean weapon_stabilised = trap_Cvar_VariableValue("vr_weapon_stabilised") != 0.0f;
		qboolean weapon_stabilised = player_weapon_stabilised();

		if (!show_in_hand_enabled || (two_handed_enabled && weapon_stabilised))
		{
			return;
		}
...
#if defined USE_TRAIL_ITEM
		if (ci->newAnims && cent->currentState.clientNum != cg.snap->ps.clientNum) {
...
static void CG_PlayerFloatSprite( const centity_t *cent, qhandle_t shader ) {
	...
	if ( cent->currentState.number == cg.snap->ps.clientNum && ( cg.vr_controller_type || !cg.renderingThirdPerson) ) {//TODO check the "followed player" sprite
...
static void CG_PlayerSprites( centity_t *cent ) {
	...
	// we don't show the talk ballon anymore as virtual menu is a good indicator.
	// GUNNM TODO create cvar_t to force talk balloon sprite instead of the virtual menu
	...
	if (cent->currentState.number == cg.snap->ps.clientNum && cg.renderingThirdPerson && CG_IsThirdPersonFollowMode_Query()) {
...
#if defined USE_DEATHCAM && defined USE_NATIVE_HACK
...
qboolean CG_LaserNeeded( centity_t *cent, qboolean from_cg_player ) {
	...
	clientInfo_t	*ci;
	weapon_t		weaponNum;
#if defined USE_WEAPON_WHEEL || defined EF_LASER_SIGHT
	int vrFlags;
#endif
	...
	//isDead	= (cent->currentState.eFlags & EF_DEAD);
	inMenu		= (cent->currentState.eFlags & EF_TALK);
	isFiring	= (cent->currentState.eFlags & EF_FIRING);
	weaponNum	= cent->currentState.weapon;

	// always show laser beam in menu 
	if ( mySelf && inMenu )
		return qtrue;

#if defined USE_WEAPON_WHEEL || defined EF_LASER_SIGHT
	vrFlags = VR_get_cent_vrFlags( cent, mySelf );
#endif

#ifdef USE_WEAPON_WHEEL
	if ( mySelf && vrFlags & EF_WEAPON_WHEEL )
		return qfalse;
#ifdef USE_VR_QVM
	else if ( vrFlags & EF_WEAPON_WHEEL )
		return qfalse;
#endif
#endif

	// no laser beam with some weapons
	if (weaponNum == WP_GAUNTLET || weaponNum == WP_GRENADE_LAUNCHER)
		return qfalse;

	// no laser beam during fire with some weapons
	if (isFiring && (weaponNum == WP_LIGHTNING ||
...
void CG_Player( centity_t *cent ) {
	...
	if ( mySelf && !isDead && !cg.renderingThirdPerson )
		renderfx = RF_THIRD_PERSON;

	if ( cg_deadBodyDarken.integer && isDead )
		darken = qtrue;
	else
		darken = qfalse;
...
#if defined USE_DEATHCAM && defined USE_NATIVE_HACK
	if ( shadow && ((mySelf && cg_playerShadow.integer == 3) || (!mySelf && cg_shadows.integer == 3)) ) {
#else
	...
	// the torso will be add later, because we need to change
	// its placement/axis depending of the gun position/axis

	if ( ci->vr_controller_type == 1 )
	{
		//=================
		//		3 Dof
		//=================
		// weapon axis must be exact vr controller axis.
		// but it is place on, and therefore after, the torso.
		// we then need to :
		// - subtract VR controller axis from gun axis 
		// - add the result to torso.axis

		// get the weapon angle, from torso placement and axis. we will later remove these angles from the torso matrix
		CG_GetPlayerWeaponAxis(&torso, NULL, cent, &gun);

		// we want the gun to have the vr controller exact angles
		// so the torso must do the appropriate rotations 
		if ( !isDead || inMenu )
		{
			vec3_t res_axis;
			...
			// add the [PITCH] axis
			VectorAdd(torso.axis[PITCH], res_axis, torso.axis[PITCH]);

			memcpy( gun.axis, vr_controller_axis, 36 );
		}
	}
	else if ( ci->vr_controller_type == 2 )
	{
		//=================
		//		6 Dof
		//=================
		if (!isDead || inMenu) {
			AnglesToAxis(rightAngles, torso.axis);
		}
	}
	...
	if ( (cent->currentState.powerups & ( 1 << PW_INVULNERABILITY ) ) ||
		...
		powerup.frame = 0;//TODO is it usefull?
		powerup.oldframe = 0;// usefull?
		powerup.customSkin = 0;
		// always draw
	...
	if ( ci->medkitUsageTime && t < 500 ) {
		...
		powerup.frame = 0; // was not in q3e
		powerup.oldframe = 0; // was not in q3e
		powerup.customSkin = 0;
		// always draw
		powerup.renderfx &= ~RF_THIRD_PERSON;
		VectorClear(angles);
	...
	if ( mySelf && !isDead && cg.renderingThirdPerson && ci->vr_controller_type )
		head.renderfx = RF_THIRD_PERSON; // show head only in mirrors in VR
	else
...
#ifndef USE_DEATHCAM
	//change VR view origin when dead to follow the head
	if ( myself && isDead )
		VectorCopy( head.origin, cg.vr_vieworigin);
#endif
	...
	//=============================
	{
		int keepTorsoRenderfx = torso.renderfx;
		if ( ci->vr_controller_type == 1 && mySelf && !isDead )
		{
			// hide the torso when leaning depending on cvar VR_hide_torso.
			if ( rightAngles[PITCH] >= VR_angle_hide_torso.integer || VR_hide_torso.integer )
				torso.renderfx = RF_THIRD_PERSON; // only show in mirror
		}

		CG_AddRefEntityWithPowerups(&torso, &cent->currentState, ci->team);

		// don't change visibility of weapon (who used parent.rendefx) 
		if ( rightAngles[PITCH] >= VR_angle_hide_torso.integer || VR_hide_torso.integer )
			torso.renderfx = keepTorsoRenderfx;
	}

	//=============================
	// add gun / barrel / flash
	//=============================
	CG_AddPlayerWeapon( &torso, NULL, cent, ci->team, &gun );
	...
	//=============================
	//	  add players laser beam
	//		or crosshair
	//=============================
#ifdef USE_LASER_SIGHT
	laserNeeded = CG_LaserNeeded(cent, qtrue);
	if ( mySelf && !laserNeeded && !weapon_select )
#endif
		CG_DrawCrosshair3D(cent);

	//=============================
	//		add 3D menu geometry
	//=============================
	...
	if ( mySelf && ci->vr_controller_type && showVirtualKeyboard.integer ) {
		CG_VR_Keyboard( cent );
	}
#endif
}
