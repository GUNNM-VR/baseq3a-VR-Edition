...
#ifdef USE_VR_QVM
			if ( out->vr_controller_type ) {
				// HeadSet position
				out->HMD_raw_origin[i] = next->ps.HMD_raw_origin[i];

				// Right hand angles
				out->right_hand_angles[i] = LerpAngle(
					prev->ps.right_hand_angles[i], next->ps.right_hand_angles[i], f);

				// Right hand weapon muzzle point
				out->right_muzzle_position[i] = next->ps.right_muzzle_position[i];

				// Right hand position
				out->right_hand_position[i] = next->ps.right_hand_position[i];

				// left hand angles
				out->left_hand_angles[i] = LerpAngle(
					prev->ps.left_hand_angles[i], next->ps.left_hand_angles[i], f);

				// left hand position
				out->left_hand_position[i] = next->ps.left_hand_position[i];
			}
#endif // USE_VR_QVM
		}
...
void CG_PredictPlayerState( void ) {
	...
	if ( oldestCmd.serverTime > cg.snap->ps.commandTime
		...
		//We want to keep free look in VR during intermission
#ifdef USE_VR
...
/*#ifndef USE_VR
#ifdef USE_VR_QVM
		//if (!cg.snap->ps.vr_controller_type && cg.snap->ps.pm_type != PM_INTERMISSION ) {
		if ( !cg.snap->ps.vr_controller_type )
#endif // USE_VR_QVM
			return;
#endif // USE_VR*/
	}
