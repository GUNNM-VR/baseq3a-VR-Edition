...
void CG_AdjustFrom640( float *x, float *y, float *w, float *h )
{
	...
	float screenXBias	= cgs.screenXBias;
	float screenYBias	= cgs.screenYBias;
	int xoffset			= 0;
	int yoffset			= 0;
	const auto depth	= 3;
	//const auto depth	= (int)trap_Cvar_VariableValue("vr_hudDepth"); // TODO

	float scaleX = 2.0f;
	float scaleY = scaleX * 480.0f / 640.0f;
...
#endif
	// scale for screen sizes
	*x = *x * screenXScale + screenXBias;
	*y = *y * screenYScale + screenYBias;
	*w *= screenXScale;
	*h *= screenYScale;
#ifdef USE_VR
	// Recenter scaled screen
	if ( cg.vr_controller_type )
	{
#ifdef USE_HUD_FLAG
		...
		{
			yoffset = -100.0f;
			//yoffset = - trap_Cvar_VariableValue("vr_hudYOffset"); // TODO
			xoffset = 120 - (depth * 20);

			if (cg.stereoView == STEREO_RIGHT) {
				xoffset *= -1;
			}

			*x += (cg.refdef.width  - (640 * screenXScale)) * 0.5f + xoffset;
			*y += (cg.refdef.height - (480 * screenYScale)) * 0.5f + yoffset;
		}
	}
#endif
}
...
void CG_DrawString( float x, float y, const char *string, const vec4_t setColor, float charWidth, float charHeight, int maxChars, int flags ) 
{
	...
	//trap_R_SetColor( NULL );// already commented out in q3e
}
#else
