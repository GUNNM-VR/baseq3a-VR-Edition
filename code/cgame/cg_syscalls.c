...
// keep track of the weapon muzzle origin in order to draw the laser beam
// and to get its intersection with the 2D elements.
void 	trap_VR_keepRightMuzzlePos( int x, int y, int z ) {
	syscall( CG_KEEP_RIGHT_POS, x, y, z );
}

// send angles to client
// TODO client already have vr controller angles
void 	trap_VR_KeepRightAngles( vec3_t angles ) {
	syscall( CG_KEEP_RIGHT_ANGLES, angles );
}

void 	trap_VR_keepLeftMuzzlePos( vec3_t pos ) {
	syscall(CG_KEEP_LEFT_POS, pos[0], pos[1], pos[2]);
}

// TODO : send angles to client, but client already have those angles
void 	trap_VR_KeepLeftAngles( vec3_t angles ) {
	syscall( CG_KEEP_LEFT_ANGLES, angles );
}
#endif
...
#ifdef USE_NEOHUD
// Let the client know we are now rendering the HUD and/or some 2D parts.
// This can be use to draw the HUD in a separate texture
void	trap_R_RenderHUD( const refdef_t *fd ) {
	syscall( CG_R_RENDERHUD, fd );
}
#endif
