...
#define HOLDABLE_ITEM_SOFT_LIMIT	60 // fixme: find appropriate limit
...
static int HUD_SoftLimit(int property, int param) {
	...
	case VAL_FPS:
	{
		int com_maxfps = (int)CG_Cvar_Get( "com_maxfps" );
		return com_maxfps;
		// or use magic number as soft value ?
		// return 240;
	}
	case VAL_SPEED:
...
static qboolean check_vis_gen_cases(const unsigned int visArray[2], int arrayIdx) {
	...
	if (inMenu) {
#ifndef USE_NEOHUD // deactivated with USE_NEOHUD : easier to choose a HUD file
...
void copyColor(vec4_t colSource, vec4_t *colDest) {
	memcpy(colDest, colSource, sizeof(vec4_t));
}
...
void HUD_DrawBackground(item_t * itm, rectangle_t *r) {
	...
	if (itm->iconFlags & ICON_GAME_TYPE) {
		...
		shader = cgs.media.blueFlagShader[cgs.blueflag];
	}
...
//GUNNM TODO if caption, use str lenght and charW according to align, valign, and charH
void HUD_Update_finalRect(item_t *itm, int x, int y, int w, int h) {
	...
	//display all the finalRect, debug use only:
...
=================
*/
static qboolean CheckAlpha(int handle, color_Def_t *col, float *alpha) {
	if (needCacheToken)// do we need this test?
		return qfalse;
...
// TODO factorize next 2 functions
// forecolor <vec4_t>
