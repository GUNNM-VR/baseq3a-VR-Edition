...
//=================================================
// for VR keep gun position / axes / muzzle / ROLL
typedef struct client_weapon_s {
	//float		origin[3]; // no more used
	vec3_t		axis[3];
	float		muzzle[3];
	vec3_t		vr_controller_3axis_smooth;
} client_weapon_t;

extern client_weapon_t client_weapon;
...
typedef struct {
...
#ifdef USE_VR//_QVM
	vec3_t		refdefVR_RightAngles; // right hand VR controller
#endif
	// zoom key
	qboolean	zoomed;
	int			zoomTime;
	...
	float 			mouseX;
	float 			mouseY;
	float 			heightStand;	// dynamic height view from model stand up position
	float 			heightCrouch;	// dynamic height view from model crouch position
	stereoFrame_t	stereoView;

	// 0 : PC / Android / any standart 2D screen
	// 1 : Oculus GO / Samsung Gear / any OpenXR 3Dof headset
	// 2 : Quest I & II / any OpenXR 6Dof headset
	int				vr_controller_type;
	float 			worldscale;
	vec3_t 			vr_vieworigin;	// view origin in VR thirdperson  & view follow head when dead
#endif
...
typedef struct {
	...
	//reuse missionpack nail gun sounds for the grappling hook
...
#endif
#ifdef USE_DEATHCAM
extern  vmCvar_t		cg_allowDeathCam;
#endif
...
extern	vmCvar_t		showVirtualKeyboard;
extern	vmCvar_t		VR_hide_torso;
extern	vmCvar_t		VR_angle_hide_torso;
extern	vmCvar_t		menu_distance;
extern	vmCvar_t		vr_controller_type;
extern	vmCvar_t		laserBeam;

//drbeef trail item TODO
...
/*#ifdef USE_VR
int get_vr_controller_type( centity_t *cent );
#endif*/
//===============================================
...
#if defined USE_VR || defined USE_WEAPON_WHEEL || defined USE_DEATHCAM
...
#ifdef USE_NEOHUD
void		trap_R_RenderHUD(const refdef_t *fd);
void		CG_ChangeFrameResolution(int vidWidth, int vidHeight);
#endif

#ifdef USE_VR_QVM
void 		trap_VR_keepRightMuzzlePos( int x, int y, int z );
void 		trap_VR_KeepRightAngles( vec3_t angles );
void 		trap_VR_KeepSpawnAngles( vec3_t spawnAngles );
void 		trap_VR_keepLeftMuzzlePos( vec3_t pos );
void 		trap_VR_KeepLeftAngles( vec3_t angles );
#endif // USE_VR_QVM
